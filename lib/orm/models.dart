...
//endregion TransactionManager
      ...
      */
      // region Typx
      class Typx {
        Typx({this.idt, this.type,this.subType,this.natural}) { setDefaultValues();}
        Typx.withFields(this.type,this.subType,this.natural){ setDefaultValues();}
        Typx.withId(this.idt, this.type,this.subType,this.natural){ setDefaultValues();}
...
// COLLECTIONS
    ...
    // END COLLECTIONS
        ...
        Map<String, dynamic> toMap({bool forQuery=false}) {
          final map = Map<String, dynamic>();
          if (idt != null) {map["idt"] = idt;}    if (type != null) {map["type"] = type;}
    ...
    if (natural != null) {map["natural"] = natural;}
        ...
        List<dynamic> toArgs() {
          return[idt,type,subType,natural];   
        }  
        ...
        Future<int> save() async {
          if (idt == null || idt == 0) {
            idt = await _mnTypx.insert(
                Typx.withFields(type,subType,natural)); }
          else {
            idt= await _upsert(); }
          return idt;
        }
        ...
        Future<List<BoolResult>> saveAll(List<Typx> typxs) async {
          final results = _mnTypx.saveAll("INSERT OR REPLACE INTO Typx (idt, type,subType,natural)  VALUES (?,?,?,?)",typxs);
          return results;
        }
        ...
        Future<int> _upsert() async {
          idt = await _mnTypx.rawInsert(
              "INSERT OR REPLACE INTO Typx (idt, type,subType,natural)  VALUES (?,?,?,?)", [idt,type,subType,natural]);
          return idt;
        }
        ...
        Future<List<BoolResult>> upsertAll(List<Typx> typxs) async {
          final results = await _mnTypx.rawInsertAll(
              "INSERT OR REPLACE INTO Typx (idt, type,subType,natural)  VALUES (?,?,?,?)", typxs);
          return results;
        }
